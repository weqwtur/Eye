...
**Eye** — це телеграм бот про вивчення ока.  
Найбільше зосередження в сфері комфортного вивчення “ока”.
...
Блінкер. Після команди бот відправляє два повідомлення: перше це гіф під якою інлайн кнопка, а друге — кількість тиків на інлайн кнопку (або ж на oko), тобто кількість моргань.

-----

### 𖦋 `/menu` — Меню

Головна точка входу в бота. Все через інлайн клавіатуру.
...
Посилання на базу захворювань ока.  
В планах — повноцінний вбудований розділ з 40 хворобами та станами.
...
84 факти про oko.  
Перегортання через інлайн кнопки: “⭠” та “⭢”.
...
**Надіслати будь-який текст** → Oko закодує його у Брайль.  
**Надіслати послідовність Брайля** → Oko декодує назад у звичайний текст.
...
Ти сліпий до їхнього сенсу — поки не запитаєш Oko.*
...
│   └── middleware/
│       ├── i18n.py                # Мова юзера
│       └── user_register.py       # Авто-реєстрація
│
├── locales/
│   ├── uk.json
│   ├── en.json
...
│
├── handlers/
│   ├── start.py                   # /start — блінкер + клікер
│   ├── menu.py                    # /menu — inline навігація
│   ├── facts.py                   # Факти — карусель
│   ├── sense.py                   # Сенс бота
│   ├── report.py                  # Репорт — FSM
│   ├── github.py                  # GitHub
│   ├── diseases/
│   │   ├── router.py
│   │   ├── handlers.py            # Захворювання ока
│   │   └── data.py
│   ├── media/
│   │   ├── router.py
│   │   ├── handlers.py            # Архів + suggest flow
│   │   └── gallery.py             # Всі file_id
│   ├── top/
│   │   ├── router.py
│   │   └── handlers.py            # Топ 10
│   └── ciphers/
...
│
├── services/
│   ├── user_service.py            # CRUD юзерів
│   ├── disease_service.py         # Логіка хвороб
│   └── level_service.py           # XP та рівні
│
├── keyboards/
│   ├── main_menu.py
│   ├── diseases.py
...
│
└── utils/
    ├── formatters.py              # wavy ID, mention links
    └── paginator.py               # Універсальна пагінація
```
