   ...
   try:
       ...
       for block in blocks:
           ...
           if digits.isdigit():
               ...
               logger.warning(f"Не вдалося декодувати блок: {block!r}")
               decoded.append('?')

       return "".join(decoded)

   except Exception as e:
       logger.error(f"Помилка декодування Braille: {e}")
       return "Помилка декодування! Перевірте введені дані."
...
async def braille_auto(message: types.Message):
   ...
   if _is_braille_text(text):
       ...
       label = "Декодовано (Braille → Текст)"
   else:
       result = encode_text_to_braille_unicode(text)
       label = "Закодовано (Текст → Braille Unicode)"
   ...
   if _is_braille_text(text):
       ...
       results = [
           types.InlineQueryResultArticle(
               id="braille_decode",
               title="Декодувати Braille Unicode",
               description=f"Попередній перегляд: {decoded}",
               input_message_content=types.InputTextMessageContent(
                   message_text=(
                       f"<b>Декодовано (Braille → Текст):</b>\n"
       ...
       results = [
           types.InlineQueryResultArticle(
               id="braille_encode",
               title="Закодувати в Braille Unicode",
               description=f"Попередній перегляд: {encoded[:30]}...",
               input_message_content=types.InputTextMessageContent(
                   message_text=(
                       f"<b>Закодовано (Текст → Braille Unicode):</b>\n"
